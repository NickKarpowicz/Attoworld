...
@app.cell
def _(mo, pathlib):
    marco_data = pathlib.Path("/home/nick/ownCloud/AnalysisTestFiles/CalibrationFiles")
...
@app.cell
def _(aw, mo):
    lamp_options = mo.ui.dropdown(options=[e.value for e in aw.spectrum.CalibrationLampReferences])
    lamp_options
    return (lamp_options,)
...
def _(aw, lamp_options, mo):
    ...
    reference = aw.data.load_mean_spectrum_from_scarab(aw.spectrum.get_calibration_path() / lamp_options.value).to_normalized()
    return (reference,)
...
def _(aw, np, scipy):
    def generate_response_curve(
        ...
        taylor_series = (
            coefficients[1]
            + 0.5 * relative_wl * coefficients[2]
        )
        gaussian = np.exp(
    ...
    def fit_calibration_amplitude_model(
        ...
        initial_cal = generate_calibration_from_coeffs(amplitude_guess,wavelength_coeffs,measurement.wavelength)
        reference_shift = reference.to_interpolated_wavelength(
            initial_cal.corrected_wavelengths
        )
        def residual_amp(coeff):
            cal = generate_calibration_from_coeffs(coeff,wavelength_coeffs,measurement.wavelength)
            calibrated = cal.apply_to_spectrum(measurement)
            residuals = calibrated.spectrum - reference_shift.spectrum
            return residuals[
                (calibrated.wavelength > roi[0])
                & (calibrated.wavelength < roi[1])
            ]
    ...
    def generate_calibration_from_coeffs(amplitude_coeffs, wavelength_coeffs, wavelengths):
        new_wavelengths, new_freqs = get_new_wavelength(wavelengths*1e6,wavelength_coeffs)
        intensity_factors = 1.0/generate_response_curve(wavelength=new_wavelengths*1e6,coefficients=amplitude_coeffs)
        return aw.data.SpectrometerCalibration(intensity_factors=intensity_factors, corrected_wavelengths=new_wavelengths, corrected_frequencies=new_freqs)
...
@app.cell
def _(mo):
    wavelength0 = mo.ui.slider(value=0.75, start = 0, stop=3, step=1e-4, label="Wavelength shift center", show_value=True)
    wavelength_offset = mo.ui.slider(value=-0.0107, start=-1.0, stop=1.0, step=1e-4, label="Wavelength offset", show_value=True)
    wavelength_slope = mo.ui.slider(value = 2e-3, start=-1,stop=1, step = 1e-3,label="Wavelength slope", show_value=True)
...
@app.cell
def _(mo):
    amplitude_lam0 = mo.ui.slider(value=0.65, start=0, stop=3, step=1e-4, label="Amplitude correction center", show_value=True)
    amplitude_offset = mo.ui.slider(value=3.0, start=0, stop=30, step=1e-4, label="Amplitude multiplier", show_value=True)
    amplitude_slope = mo.ui.slider(value=4.0, start=-30, stop=30, step=1e-4, label="Amplitude slope", show_value=True)
    amplitude_width = mo.ui.slider(value=0.32, start=0, stop=3, step=1e-4, label="Amplitude width", show_value=True)
    amplitude_order = mo.ui.slider(value=4.0, start=2, stop=32, step=1e-4, label="Amplitude Gaussian order", show_value=True)
    wiener_noise_level = mo.ui.slider(value=0.01, start=0.0, stop=0.1, step = 1e-4, label="Noise level", show_value=True)
...
def _(
    ...
    initial_guess = np.array([amplitude_lam0.value, amplitude_offset.value, amplitude_slope.value, amplitude_width.value, amplitude_order.value])
    wavelength_guess = [wavelength0.value, wavelength_offset.value, wavelength_slope.value]
    guess_cal = generate_calibration_from_coeffs(initial_guess, wavelength_guess,direct.wavelength)
...
def _(
    ...
    print(ax.shape)
    p1 = ax[0,0].plot(direct.wavelength_nm(), direct.spectrum, label="Measurement")
    ...
    ax[0,0].legend()
    ax[0,1].plot(first_guess.wavelength_nm(), first_guess.spectrum, label="Initial guess")
    ...
    ax[0,1].legend()
    if fit_button.value:
        new_guess = fit_calibration_amplitude_model(measurement=direct, reference=reference, wavelength_coeffs=wavelength_guess, amplitude_guess=initial_guess, roi=np.array([200e-9,950e-9]))
        parametrized_cal = generate_calibration_from_coeffs(new_guess, wavelength_guess, direct.wavelength)
        calibrated = parametrized_cal.apply_to_spectrum(direct)

        ax[0,2].plot(
            calibrated.wavelength_nm(),
            calibrated.spectrum, label="Model calibrated")
        ...
        residual = B-A
        quadrature_signal = np.sqrt(direct_shifted.spectrum**2 + ref_on_new_axis.spectrum**2)
        new_weights = direct_shifted.spectrum * (calibrated.spectrum - residual)/(direct_shifted.spectrum**2 + wiener_noise_level.value)  
        new_weights = parametrized_cal.intensity_factors - direct_shifted.spectrum * residual/(direct_shifted.spectrum**2 +wiener_noise_level.value)
        adjusted_cal = aw.data.SpectrometerCalibration(intensity_factors=new_weights, corrected_frequencies=parametrized_cal.corrected_frequencies, corrected_wavelengths=parametrized_cal.corrected_wavelengths)
        second_calibration = adjusted_cal.apply_to_spectrum(direct)
        ax[1,1].plot(second_calibration.wavelength_nm(),second_calibration.spectrum, label="Calibrated")
        ax[1,1].plot(reference.wavelength_nm(), reference.spectrum, label="Reference")
        ax[1,1].set_ylabel("Intensity (Arb. unit)")
        ax[1,1].set_xlabel("Wavelength (nm)")
        ax[1,1].legend()
        ax[1,2].plot(adjusted_cal.corrected_wavelengths*1e9, adjusted_cal.intensity_factors, label="Final weights")
